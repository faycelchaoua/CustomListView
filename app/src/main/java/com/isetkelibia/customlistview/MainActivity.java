...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        maListViewPerso.setOnItemClickListener((parent, view, position, id) -> {
            // Méthode 1
            HashMap map1 = (HashMap) maListViewPerso.getItemAtPosition(position);
        ...
        maListViewPerso.setOnItemLongClickListener((parent, view, position, id) -> {
            // On récupère la "HashMap" contenant les infos de notre item (titre, description, img)
            HashMap map12 = (HashMap) maListViewPerso.getItemAtPosition(position);
            // On crée une boite de dialogue
            AlertDialog.Builder adb = new AlertDialog.Builder(MainActivity.this);
            // On attribue un titre à notre boite de dialogue
            adb.setTitle("Sélection Item");
            // On insère un message à notre boite de dialogue, et ici on affiche le titre
            // de l'item cliqué
            adb.setMessage("Votre choix : " + map12.get("titre"));
